...
TEST(job_config_test, config_data)
{
	...
	ASSERT_EQ(task2->sandbox->all_limits.size(), 2u);
	EXPECT_NO_THROW(task2->sandbox->all_limits.at("group1"));
	EXPECT_NO_THROW(task2->sandbox->all_limits.at("group2"));

	auto limit1 = task2->sandbox->all_limits.at("group1");
	...
	auto limit2 = task2->sandbox->all_limits.at("group2");
