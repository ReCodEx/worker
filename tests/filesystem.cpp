...
TEST(filesystem_test, normalize)
{
	...
	ASSERT_EQ(fs::path("/").string(), helpers::normalize_path(fs::path("/")).string());
	ASSERT_EQ((fs::path("/first") / fs::path("second") / fs::path("third")).string(), helpers::normalize_path(fs::path("/first/second/third")).string());
	ASSERT_EQ((fs::path("first") / fs::path("second") / fs::path("third")).string(), helpers::normalize_path(fs::path("first/second/third")).string());
	ASSERT_EQ((fs::path("/second") / fs::path("third")).string(), helpers::normalize_path(fs::path("/first/../second/third")).string());
	...
	ASSERT_EQ((fs::path("/first") / fs::path("second") / fs::path("third")).string(), helpers::normalize_path(fs::path("/first/./second/third")).string());
	ASSERT_EQ((fs::path("/first") / fs::path("second") / fs::path("third")).string(), helpers::normalize_path(fs::path("/first//second/third")).string());
	ASSERT_EQ((fs::path("/first") / fs::path("third")).string(), helpers::normalize_path(fs::path("/first////second/../third")).string());
	ASSERT_EQ((fs::path("/first") / fs::path("third")).string(), helpers::normalize_path(fs::path("/first////second/../third/././.")).string());
}
...
TEST(filesystem_test, test_absolute_3)
{
	...
	ASSERT_EQ((fs::path("/path/outside/sandbox") / fs::path("test1") / fs::path("output.stderr")).string(), result.string());
}
...
TEST(filesystem_test, test_absolute_4)
{
	...
	ASSERT_EQ((fs::path("/path/outside/sandbox") / fs::path("test1") / fs::path("sub") / fs::path("output.stderr")).string(), result.string());
}
...
TEST(filesystem_test, test_absolute_5)
{
	...
	ASSERT_EQ((fs::path("/another/path/outside/sandbox") / fs::path("sub") / fs::path("output.stderr")).string(), result.string());
}
