						   ...
						   const std::string &password, std::shared_ptr<spdlog::logger> logger)
{
	...
	http_man_ = http_man_ptr(new http_manager{username, password, logger});
}
...
void file_manager::get_file(const std::string &src_name, const std::string &dst_path)
{
	...
	http_man_->get_file(src_name, cache_man_->get_caching_dir() + src_name);
	cache_man_->get_file(src_name, dst_path);
}

void file_manager::put_file(const std::string &src_name, const std::string &dst_path)
{
	http_man_->put_file(src_name, dst_path);
}

void file_manager::set_params(const std::string &username, const std::string &password)
{
	http_man_->set_params(username, password);
}
