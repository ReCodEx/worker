...
external_task::external_task(const create_params &data)
	: task_base(data.id, data.task_meta), worker_id_(data.worker_id), sandbox_(nullptr), logger_(data.logger),
	  temp_dir_(data.temp_dir)
{
	sandbox_check();
}
...
#ifndef _WIN32
	if (task_meta_.sandbox->name == "isolate") {
		sandbox_ = std::make_shared<isolate_sandbox>(*task_meta_.sandbox->limits, worker_id_, temp_dir_, logger_);
	}
#endif
}
...
{
	return task_meta_.sandbox->limits;
}
