...
namespace bpp
{
	...
	public:
		...
		Logger &fatal()
		{
			return setSeverity(LogSeverity::FATAL);
		}
		Logger &error()
		{
			return setSeverity(LogSeverity::ERROR);
		}
		Logger &warning()
		{
			return setSeverity(LogSeverity::WARNING);
		}
		Logger &info()
		{
			return setSeverity(LogSeverity::INFO);
		}
		Logger &notice()
		{
			return setSeverity(LogSeverity::NOTICE);
		}
		Logger &debug()
		{
			return setSeverity(LogSeverity::DEBUG);
		}
		 ...
		 */
		template <typename T> Logger &write(const T &data)
		{
			mAccumulator << data;
		...
		void flush()
		{
			...
			std::size_t limitSize = applySizeLimit(
				limitSeverity); // limitSize applies to limitSeverity level only, levels above are printed completely
		...
		std::size_t size(LogSeverity severity = LogSeverity::ANY) const
		{
			...
			for (auto i = (std::size_t) LogSeverity::UNDEFINED; i <= (std::size_t) severity; ++i) {
				...
				if (s == mSeverity) { size += mAccumulatorSize; }
			}
			return size;
		}
	...
	template <> Logger &Logger::operator<<<LogSeverity>(const LogSeverity &data)
	{
