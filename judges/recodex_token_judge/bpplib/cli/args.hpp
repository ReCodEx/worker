...
namespace bpp
{
	...
	public:
		...
		public:
				...
				: mName(name), mComment(comment), mMandatory(mandatory), mPresent(false)
			{
				...
				if (mName[0] == '-') throw(ArgumentException() << "Argument name must not start with dash '-'.");
			}
		...
		public:
			ArgBool(const std::string &name, const std::string &comment) : ArgBase(name, comment, false)
			{
			}
		...
		protected:
			...
			value_t processInt(int &argc, const char **(&argv)) const
			{
				...
				if (!isInt(argv[0]))
					throw(ArgumentException() << "Value '" << argv[0] << "' of argument '" << this->getName()
				...
				if (value < mMin)
					throw(ArgumentException()
						<< "Value of argument '" << this->getName() << "' is below limits (" << mMin << ").");
				if (value > mMax)
					throw(ArgumentException()
						<< "Value of argument '" << this->getName() << "' is above limits (" << mMax << ").");
		...
		public:
			ArgIntBase(const std::string &name,
			...
			{
				if (mMin > mMax)
					throw(ArgumentException() << "Argument '" << this->getName() << "' has invalid limits (min=" << mMin
											  << ", max=" << mMax << ").");
			}
		};
		...
		public:
			ArgInt(const std::string &name,
				const std::string &comment,
				bool mandatory = false,
				value_t defaultValue = 0,
				value_t min = 0,
				value_t max = std::numeric_limits<value_t>::max())
			...
			{
				static_assert(
					sizeof(std::uint64_t) >= sizeof(std::size_t), "std::size_t is larger than 64-bit unsigned int");
			...
			std::uint64_t getAsUint() const
			{
				if (mValue < 0)
					throw(bpp::ArgumentException()
						<< "Unable to convert int argument '" << this->getName() << "' to 64-bit unsigned int.");
			...
			std::int32_t getAsInt32() const
			{
				if (mValue > (value_t) std::numeric_limits<std::int32_t>::max())
					throw(bpp::ArgumentException()
						<< "Unable to convert int argument '" << this->getName() << "' to 32-bit int.");
			...
			std::uint32_t getAsUint32() const
			{
				if (mValue > (value_t) std::numeric_limits<std::uint32_t>::max())
					throw(bpp::ArgumentException()
						<< "Unable to convert int argument '" << this->getName() << "' to 32-bit unsigned int.");
		...
		public:
			ArgIntList(const std::string &name,
				const std::string &comment,
				bool mandatory = false,
				value_t min = 0,
				value_t max = std::numeric_limits<value_t>::max())
		...
		public:
			ArgFloatBase(const std::string &name,
				const std::string &comment,
				bool mandatory = false,
				value_t min = 0.0,
				value_t max = 1.0)
		...
		public:
			ArgFloat(const std::string &name,
				const std::string &comment,
				bool mandatory = false,
				value_t defaultValue = 0.0,
				value_t min = 0.0,
				value_t max = 1.0)
		...
		public:
			ArgFloatList(const std::string &name,
				const std::string &comment,
				bool mandatory = false,
				value_t min = 0.0,
				value_t max = 1.0)
		...
		public:
			ArgString(const std::string &name,
				const std::string &comment,
				bool mandatory = false,
		...
		protected:
			...
			void process(int &argc, const char **(&argv)) override
			{
				...
				if (mOptions.find(mNormalizedValue) == mOptions.end())
					throw(ArgumentException()
						<< "Invalid value '" << this->getValue() << "' for argument '" << this->getName() << "'.");
			}

		public:
			ArgEnum(const std::string &name,
				const std::string &comment,
				bool mandatory = false,
			...
			{
				std::for_each(
					options.begin(), options.end(), std::bind(&ArgEnum::addOption, this, std::placeholders::_1));

				if (!this->getValue().empty()) mOptions.insert(this->getValue());
			}
			...
			{
				std::string lower;
				if (!mCaseSensitive) { lower = toLower(value); }

				if (this->getValue() == (mCaseSensitive ? value : lower)) return true;
		...
		protected:
			void process(int &argc, const char **(&argv)) override
			{
				...
				while (
					argc && (argv[0][0] != '-' || (argv[0][0] && argv[0][1] != '-' && !args->isSingleDashAllowed()))) {
					mValues.push_back(argv[0]);
					--argc;
					++argv;
				}
	...
	private:
		...
		bool mAllowSingleDash; ///< Whether single dash is allowed as attribute prefix (normally, two dashes prefix long
							   ///< arguments)
		 ...
		 */
		template <typename ARG_T> ARG_T &getArgTyped(const std::string &name)
		{
			...
			if (arg == nullptr) throw(ArgumentException() << "Argument '" << name << "' type mismatch.");
			return *arg;
		}
		 ...
		 * \throws ArgumentException if the argument is not found or has invalid type.
		 */
		template <typename ARG_T> const ARG_T &getArgTyped(const std::string &name) const
		{
			if (mArguments.find(name) == mArguments.end())
				throw(ArgumentException() << "Argument '" << name << "' was not specified.");
	...
	public:
			...
			: mNamelessMin(namelessMin), mNamelessMax(namelessMax), mAllowSingleDash(false)
		{
			if (namelessMin > namelessMax)
				throw(ArgumentException() << "Nameless arguments minimum (" << namelessMin << ") exceeds the maximum ("
										  << namelessMax << ").");
		}
		 ...
		 */
		bool isSingleDashAllowed() const
		{
			return mAllowSingleDash;
		}
	...
	{                                                                                                                  \
		...
		const std::string &operator[](std::size_t idx) const
		{
			if (idx >= mNamelessArguments.size())
				throw(ArgumentException() << "Unable to retieve nameless argument #" << idx << " (only "
										  << mNamelessArguments.size() << " provided).");
		...
		{
			if (mNamelessCaptions.size() <= idx) mNamelessCaptions.resize(idx + 1);
		...
		{
			if (!arg) { throw(RuntimeError("Argument object must not be NULL.")); }
		...
		void process(int argc, const char *argv[])
		{
			...
			while (argc > 0 && argv[0] && argv[0][0] == '-' && (mAllowSingleDash || argv[0][1] == '-')) {
				...
				if (!name[0]) break; // empty name ~ end of named argument;

				// Find corresponding argument object.
				auto argIt = mArguments.find(name);
				if (argIt == mArguments.end()) throw(ArgumentException() << "Unknown argument '" << name << "'.");
			...
			for (auto it = mArguments.begin(); it != mArguments.end(); ++it) it->second->checkConstraints(mArguments);
