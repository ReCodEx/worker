...
namespace bpp
{
	...
	public:
		...
		template <typename T> FileError &operator<<(const T &data)
		{
	...
	public:
		...
		{
			if (path.empty()) throw RuntimeError("Empty path provided to crop_extension function.");
			...
			if (pos == std::string::npos || (lastSlashPos != std::string::npos && pos < lastSlashPos)) return path;
		...
		static bool exists(const std::string &path)
		{
			...
			if (stat(path.c_str(), &fileStatus) == 0) return true;
		...
		static bool isRegularFile(const std::string &path)
		{
			...
			return ((attrs & FILE_ATTRIBUTE_DIRECTORY) == 0) && ((attrs & FILE_ATTRIBUTE_DEVICE) == 0) &&
		...
		{
			if (std::remove(path.c_str()) != 0) throw(FileError() << "Unable to remove '" << path << "'.");
		}
	};
